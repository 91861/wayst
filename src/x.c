...
void
WindowX11_set_fullscreen(struct WindowBase* self, bool fullscreen);

void
WindowX11_resize(struct WindowBase* self, uint32_t w, uint32_t h);

void
WindowX11_events(struct WindowBase* self);

void
WindowX11_set_wm_name(struct WindowBase* self, const char* title);

void
WindowX11_set_title(struct WindowBase* self, const char* title);

void
WindowX11_set_swap_interval(struct WindowBase* self, int32_t ival);

void
WindowX11_maybe_swap(struct WindowBase* self);

void
WindowX11_destroy(struct WindowBase* self);

int
WindowX11_get_connection_fd(struct WindowBase* self);

void
WindowX11_clipboard_get(struct WindowBase* self);

void
WindowX11_clipboard_send(struct WindowBase* self, const char* text);
...
static void
WindowX11_setup_pointer(struct WindowBase* self)
{
    XColor c = { .red = 0, .green = 0, .blue = 0 };
    static char data[8] = { 0 };

    Pixmap pmp = XCreateBitmapFromData(
      globalX11->display, windowX11(self)->window, data, 8, 8);
...
static void
WindowX11_pointer(struct WindowBase* self, bool hide)
{
    if (hide && !FLAG_IS_SET(self->state_flags, WINDOW_POINTER_HIDDEN)) {

        XDefineCursor(globalX11->display,
                      windowX11(self)->window,
                      globalX11->cursor_hidden);
...
WindowX11_new(uint32_t w, uint32_t h)
{
    ...
    struct WindowBase* win = calloc(
      1, sizeof(struct WindowBase) + sizeof(WindowX11) - sizeof(uint8_t));
    ...
    glXMakeCurrent(
      globalX11->display, windowX11(win)->window, windowX11(win)->glx_context);
    ...
    XSetWMProtocols(
      globalX11->display, windowX11(win)->window, &globalX11->wm_delete, 1);
    ...
    XkbSelectEvents(globalX11->display,
                    XkbUseCoreKbd,
                    XkbAllEventsMask,
                    XkbAllEventsMask);
...
static inline void
WindowX11_fullscreen_change_state(struct WindowBase* self, const long arg)
{
...
WindowX11_resize(struct WindowBase* self, uint32_t w, uint32_t h)
{
    ...
    XConfigureWindow(globalX11->display,
                     windowX11(self)->window,
                     CWWidth,
                     &changes);
...
WindowX11_events(struct WindowBase* self)
{
    while (XPending(globalX11->display)) {
        ...
        case KeyPress:;
            ...
            KeySym ret;
            char buf[5] = { 0 };
            Xutf8LookupString(globalX11->ic,
                              &e->xkey,
                              buf,
                              4,
                              &ret,
...
void
WindowX11_set_current_context(struct WindowBase* self)
{ }
