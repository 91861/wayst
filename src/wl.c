...
/* Seat listener */
static void seat_test_capabilities(void*           data,
                                   struct wl_seat* seat,
                                   uint32_t        caps)
{
...
static struct wl_seat_listener seat_listener = { .capabilities =
                                                   seat_test_capabilities,
...
static void data_source_handle_dnd_drop_performed(
  void*                  data,
  struct wl_data_source* wl_data_source)
{
...
static void cursor_set(struct wl_cursor* what, uint32_t serial)
{
    ...
    wl_pointer_set_cursor(globalWl->pointer, serial, globalWl->cursor_surface,
...
{
    global =
      calloc(1, sizeof(WindowStatic) + sizeof(GlobalWl) - sizeof(uint8_t));
    ...
    if (globalWl->data_device_manager) {
        globalWl->data_device = wl_data_device_manager_get_data_device(
          globalWl->data_device_manager, globalWl->seat);

        wl_data_device_add_listener(globalWl->data_device,
                                    &data_device_listener, win);
    }
    ...
    eglChooseConfig(globalWl->egl_display, cfg_attribs, &config, 1,
                    &num_config);
    ...
    eglSurfaceAttrib(globalWl->egl_display, windowWl(win)->egl_surface,
    ...
    if (globalWl->xdg_shell) {
        ...
        xdg_surface_add_listener(windowWl(win)->xdg_surface,
                                 &xdg_surface_listener, win);

        windowWl(win)->xdg_toplevel =
          xdg_surface_get_toplevel(windowWl(win)->xdg_surface);

        xdg_toplevel_add_listener(windowWl(win)->xdg_toplevel,
    ...
    eglMakeCurrent(globalWl->egl_display, windowWl(win)->egl_surface,
                   windowWl(win)->egl_surface, windowWl(win)->egl_context);
...
{
    eglMakeCurrent(globalWl->egl_display, EGL_NO_SURFACE, EGL_NO_SURFACE,
                   EGL_NO_CONTEXT);
}
...
static void WindowWl_swap_buffers(struct WindowBase* self)
{
    self->paint = false;
    EGLBoolean result =
      eglSwapBuffers(globalWl->egl_display, windowWl(self)->egl_surface);
