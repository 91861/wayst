 ...
 *      DEF_VECTOR(Thing, ThingContext_destroy_Thing, ThingContext)
...
#define DEF_VECTOR(t, dtor, ...)                                                                   \
    ...
    {                                                                                              \
        size_t cap, size;                                                                          \
        t*     buf;                                                                                \
        __VA_OPT__(void* dtor_arg);                                                                \
    } Vector_##t;                                                                                  \
                                                                                                   \
    static inline Vector_##t Vector_new_##t(__VA_OPT__(__VA_ARGS__* destroy_arg))                  \
    {                                                                                              \
        return (Vector_##t){ .cap  = 4,                                                            \
                             .size = 0,                                                            \
                             .buf =                                                                \
                               malloc(sizeof(t) << 2) __VA_OPT__(, .dtor_arg = destroy_arg) };     \
    }                                                                                              \
                                                                                                   \
    static inline Vector_##t Vector_new_with_capacity_##t(                                         \
      size_t init_cap __VA_OPT__(, __VA_ARGS__* destroy_arg))                                      \
    {                                                                                              \
        return (Vector_##t){ .cap  = init_cap,                                                     \
                             .size = 0,                                                            \
                             .buf  = malloc(sizeof(t) * init_cap)                                  \
                               __VA_OPT__(, .dtor_arg = destroy_arg) };                            \
    }                                                                                              \
                                                                                                   \
    ...
    {                                                                                              \
        if (dtor)                                                                                  \
            for (size_t i = idx; i < n + idx; ++i)                                                 \
                ((void (*)(__VA_OPT__(__VA_ARGS__*, ) t*))dtor)(                                   \
                  __VA_OPT__(self->dtor_arg, ) self->buf + i);                                     \
    ...
    {                                                                                              \
        ...
        if (dtor)                                                                                  \
            for (size_t i = 0; i < n && self->size > 0; ++i)                                       \
                ((void (*)(__VA_OPT__(__VA_ARGS__*, ) t*))dtor)(                                   \
                  __VA_OPT__(self->dtor_arg, ) self->buf + --self->size);                          \
        else                                                                                       \
    ...
    {                                                                                              \
        if (dtor) {                                                                                \
            for (size_t i = 0; i < self->size; ++i)                                                \
                ((void (*)(__VA_OPT__(__VA_ARGS__*, ) t*))dtor)(__VA_OPT__(self->dtor_arg, ) &     \
                                                                self->buf[i]);                     \
        }                                                                                          \
        free(self->buf);                                                                           \
        self->buf = NULL;                                                                          \
