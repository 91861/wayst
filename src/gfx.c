...
static Atlas Atlas_new(FT_Face face_)
{
    ...
    for (int i = ATLAS_RENDERABLE_START; i < ATLAS_RENDERABLE_END; i++) {
        if (FT_Load_Char(face_, i,
                         lcd_filter ? FT_LOAD_TARGET_LCD
                                    : FT_LOAD_TARGET_MONO) ||
            FT_Render_Glyph(face_->glyph, lcd_filter ? FT_RENDER_MODE_LCD
                                                     : FT_RENDER_MODE_MONO)) {
            WRN("font error");
        }
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, ox, oy, char_width, char_height,
                        GL_RGB, GL_UNSIGNED_BYTE, g->bitmap.buffer);
...
Cache_get_glyph(Cache* self, FT_Face face, char32_t code)
{
    ...
    if (FT_Load_Glyph(face, index, FT_LOAD_TARGET_LCD) ||
        ...
        if (FT_Load_Glyph(face_fallback, index, FT_LOAD_TARGET_LCD) ||
            ...
            } else if ((e = FT_Render_Glyph(face_fallback2->glyph,
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, color ? GL_RGBA : GL_RGB,
                 (tex.w = g->bitmap.width / (lcd_filter && !color ? 3 : 1)),
                 (tex.h = g->bitmap.rows), 0,
...
void gfx_resize(uint32_t w, uint32_t h)
{
    ...
    pen_begin = +sy * (height / 64.0 / 1.75) + sy * ((hber) / 2.0 / 64.0);
    pen_begin_pixels =
    ...
    glVertexAttribPointer(bg_shader.attribs->location, 2, GL_FLOAT, GL_FALSE, 0,
                          0);

    float bg_box[] = {
        0.0f,        0.0f,        0.0f,        line_height,
        glyph_width, line_height, glyph_width, 0.0f,
    };

    glBufferData(GL_ARRAY_BUFFER, sizeof bg_box, bg_box, GL_STREAM_DRAW);
}

__attribute__((always_inline)) inline Pair_uint32_t gfx_get_char_size(
  void* self)
{
...
Pair_uint32_t gfx_pixels(void* self, uint32_t c, uint32_t r)
{
    static uint32_t gw = 0;
    if (!gw) {
        gw = face->glyph->advance.x;
...
void gfx_load_font()
{
    ...
    if (FT_Set_Char_Size(face, settings.font_size * 64, settings.font_size * 64,
    ...
    if (settings.font_name_bold) {
        ...
        if (FT_Set_Char_Size(face_bold, settings.font_size * 64,
    ...
    if (settings.font_name_italic) {
        ...
        if (FT_Set_Char_Size(face_italic, settings.font_size * 64,
                             settings.font_size * 64, settings.font_dpi,
                             settings.font_dpi)) {
            LOG("Failed to set font size\n");
        }
    ...
    if (settings.font_name_fallback) {
        ...
        if (FT_Set_Char_Size(face_fallback, settings.font_size * 64,
                             settings.font_size * 64, settings.font_dpi,
                             settings.font_dpi)) {
            LOG("Failed to set font size\n");
        }
...
void gfx_init()
{
    ...
    glUniform3f(
      font_shader.uniforms[1].location, ColorRGB_get_float(settings.fg, 0),
    ...
    squiggle_texture = create_squiggle_texture(t_height * M_PI / 2.0, t_height,
                                               CLAMP(t_height / 7, 1, 10));
}

bool gfx_set_focus(const bool focus)
{
    bool ret = false;
    if (in_focus && !focus)
        ret = true;
...
void gfx_notify_action(void* self)
{
    blink_switch  = TimePoint_ms_from_now(settings.text_blink_interval);
    draw_blinking = true;
    recent_action = true;
...
bool gfx_update_timers(Vt* vt)
{
    ...
    if (vt->scrollbar.visible) {
        if (scrollbar_fade < SCROLLBAR_FADE_MAX) {
            scrollbar_fade =
              MIN(scrollbar_fade + SCROLLBAR_FADE_INC, SCROLLBAR_FADE_MAX);
            repaint = true;
        }
    } else {
        if (scrollbar_fade > SCROLLBAR_FADE_MIN) {
            scrollbar_fade =
              MAX(scrollbar_fade - SCROLLBAR_FADE_DEC, SCROLLBAR_FADE_MIN);
            repaint = true;
        }
    }
  ...
  uint_fast16_t line_index)
{
    ...
    if (vt_line->proxy.data[0]) {
        ...
        Vector_push_GlyphBufferData(
          vec_glyph_buffer,
          (GlyphBufferData){ {
            { -1.0f, tex_begin_y + line_height, 0.0f, 0.0f },
            { -1.0, tex_begin_y, 0.0f, 1.0f },
            { tex_end_x, tex_begin_y, 1.0f, 1.0f },
            { tex_end_x, tex_begin_y + line_height, 1.0f, 0.0f },
          } });
    }
}

static uint_fast32_t                                   quad_index;
__attribute__((hot, always_inline)) static inline void gfx_draw_line_quads(
  VtLine* const vt_line)
{
...
__attribute__((hot)) static inline void gfx_rasterize_line(const Vt* const vt,
                                                           VtLine*      vt_line,
                                                           const size_t line,
...
#define BOUND_RESOURCES_IMAGE 4
    ...
    Framebuffer_generate_texture_attachment(&line_fb, texture_width,
                                            texture_height);
    ...
    for (size_t i = 0; i <= vt_line->data.size; ++i) {
        ...
        if (i == length ||
            ...
            if (!ColorRGBA_eq(bg_color, settings.bg)) {
                ...
                buffer[4] = buffer[6] =
                  -1.0f + (i + extra_width) * scalex *
                            glyph_width_pixels; // set buffer end

                if (bound_resources != BOUND_RESOURCES_BG) {
                    glBindBuffer(GL_ARRAY_BUFFER, line_bg_vao.vbo);
                    glVertexAttribPointer(bg_shader.attribs->location, 2,
                        ...
                        { // for each block with the same fg color
                            ...
                            for (const VtRune* k = r_begin; k != r; ++k) {
                                ...
                                if (z->code > ATLAS_RENDERABLE_START &&
                                    ...
                                    float x3 = -1.0f +
                                               ...
                                               l;
                                    float y3 =
                                      -1.0f + pen_begin_pixels * scaley - t;
                            ...
                            if (vec_glyph_buffer->size ||
                                ...
                                glVertexAttribPointer(
                                  font_shader.attribs->location, 4, GL_FLOAT,
                                  GL_FALSE, 0, 0);
                                ...
                                // italic
                                if (vec_glyph_buffer_italic !=
                                    vec_glyph_buffer) {
                                    glBindBuffer(GL_ARRAY_BUFFER,
                                                 flex_vbo_italic.vbo);
                                    glVertexAttribPointer(
                                      font_shader.attribs->location, 4,
                                      GL_FLOAT, GL_FALSE, 0, 0);
                                ...
                                if (unlikely(z->code > ATLAS_RENDERABLE_END)) {
                                    ...
                                    if ((z + 1 != r &&
                                        ...
                                        if (vec_glyph_buffer->size) {
                                            ...
                                            glUniform3f(
                                              font_shader.uniforms[1].location,
                                              ColorRGB_get_float(fg_color, 0),
                                              ColorRGB_get_float(fg_color, 1),
                                              ColorRGB_get_float(fg_color, 2));

                                            glUniform4f(
                                              font_shader.uniforms[2].location,
                                              ColorRGBA_get_float(bg_color, 0),
                                              ColorRGBA_get_float(bg_color, 1),
                                              ColorRGBA_get_float(bg_color, 2),
                                              ColorRGBA_get_float(bg_color, 3));
                                        ...
                                        if (vec_glyph_buffer_bold->size) {
                                            ...
                                            glVertexAttribPointer(
                                              image_shader.attribs->location, 4,
                                              GL_FLOAT, GL_FALSE, 0, 0);
            ...
            buffer[0] = buffer[2] =
              -1.0f + (i + extra_width) * scalex *
                        glyph_width_pixels; // set background buffer start
...
#define SET_BOUNDS_END(what, index)                                            \
    if (drawing[index]) {                                                      \
          ...
          -1.0f + (float)column * scalex * (float)glyph_width_pixels;          \
    }
            ...
            if (vec_vertex_buffer.size) {
                if (bound_resources != BOUND_RESOURCES_LINES) {
                    ...
                    glBindBuffer(GL_ARRAY_BUFFER, flex_vbo.vbo);
                    glVertexAttribPointer(line_shader.attribs->location, 2,
                                          GL_FLOAT, GL_FALSE, 0, 0);
                }
                ...
                ARRAY_BUFFER_SUB_OR_SWAP(vec_vertex_buffer.buf, flex_vbo.size,
                                         new_size);
            ...
            if (vec_vertex_buffer2.size) {
                ...
                glVertexAttribPointer(font_shader.attribs->location, 4,
                                      GL_FLOAT, GL_FALSE, 0, 0);

                size_t new_size = sizeof(vertex_t) * vec_vertex_buffer2.size;

                ARRAY_BUFFER_SUB_OR_SWAP(vec_vertex_buffer2.buf, flex_vbo.size,
                                         new_size);
...
__attribute__((always_inline)) static inline void gfx_draw_cursor(Vt* vt)
{
    if (!vt->cursor.hidden &&
        ((vt->cursor.blinking && in_focus) ? draw_blinking
                                           : true || recent_action)) {
            ...
            case CURSOR_BLOCK:
                if (!in_focus)
                    Vector_pushv_vertex_t(
                      ...
                      (vertex_t[4]){
                        { .x =
                            -1.0f + (col * glyph_width_pixels) * sx + 0.9f * sx,
                          .y = 1.0f - ((row + 1) * line_height_pixels) * sy +
                               0.5f * sy },
                        { .x = -1.0f + ((col + 1) * glyph_width_pixels) * sx,
                          .y = 1.0f - ((row + 1) * line_height_pixels) * sy +
        ...
        if (!filled_block) {
            ...
            glBindTexture(GL_TEXTURE_2D, 0);
            glBindBuffer(GL_ARRAY_BUFFER, flex_vbo.vbo);
            glVertexAttribPointer(line_shader.attribs->location, 2, GL_FLOAT,
                                  GL_FALSE, 0, 0);

            glUniform3f(
              line_shader.uniforms[1].location, ColorRGB_get_float(*clr, 0),
            ...
            if (cursor_char) {
                ...
                if (atlas_offset >= 0) {
                    ...
                    GlyphUnitCache* g = Cache_get_glyph(
                      source_cache, source_face, cursor_char->code);
                    h = (float)g->tex.h * sy;
                    w = (float)g->tex.w * sx;
                    t = (float)g->top * sy;
                    l = (float)g->left * sx;
                ...
                float y3 = +1.0f - pen_begin_pixels * sy -
                           (float)row * line_height_pixels * sy + t;

                vec_glyph_buffer->size = 0;
                Vector_push_GlyphBufferData(vec_glyph_buffer,
                                            (GlyphBufferData){ {
                                              { x3, y3, tc[0], tc[1] },
                                              { x3 + w, y3, tc[2], tc[1] },
                                              { x3 + w, y3 - h, tc[2], tc[3] },
                ...
                size_t newsize =
                  vec_glyph_buffer->size * sizeof(GlyphBufferData);

                ARRAY_BUFFER_SUB_OR_SWAP(vec_glyph_buffer->buf, flex_vbo.size,
                                         newsize);
...
__attribute__((always_inline)) static inline void gfx_draw_text_overlays(Vt* vt)
{
    if (vt->unicode_input.active) {
        ...
        glBindBuffer(GL_ARRAY_BUFFER, flex_vbo.vbo);
        glVertexAttribPointer(font_shader.attribs->location, 4, GL_FLOAT,
                              GL_FALSE, 0, 0);
        ...
        vertex_t lnbuf[] = {
            { -1.0f + (float)col * glyph_width_pixels * sx,
              +1.0f - pen_begin_pixels * sy -
                (float)row * line_height_pixels * sy },
        ...
        if (newsize > flex_vbo.size) {
            flex_vbo.size = newsize;
            glBufferData(GL_ARRAY_BUFFER, newsize, vec_glyph_buffer->buf,
                         GL_STREAM_DRAW);
        } else {
            glBufferSubData(GL_ARRAY_BUFFER, 0, newsize, vec_glyph_buffer->buf);
        }
...
void gfx_draw_vt(Vt* vt)
{
    ...
    glEnable(GL_SCISSOR_TEST);
    glScissor(0, win_h - gfx_get_char_size(NULL).second * line_height_pixels,
    ...
    if (vec_glyph_buffer->size) {
        ...
        glVertexAttribPointer(image_shader.attribs->location, 4, GL_FLOAT,
                              GL_FALSE, 0, 0);
    ...
    // TODO: use vbo
    if (vt->scrollbar.visible || scrollbar_fade != SCROLLBAR_FADE_MIN) {
        Shader_use(NULL);
        glBindTexture(GL_TEXTURE_2D, 0);
