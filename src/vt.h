...
static inline void
Vt_watch_fd(Vt* const self, const int fd)
{
    self->io = fd;
}
...
static inline char*
Vt_buffer(Vt* const self) { return self->out_buf; }
...
void
Vt_handle_key(void* self, uint32_t key, uint32_t mods);
...
void
Vt_handle_motion(void* self, uint32_t button, int32_t x, int32_t y);
...
static inline size_t
Vt_visual_bottom_line(const Vt* const self)
{
    return self->ws.ws_row + Vt_visual_top_line(self) + (self->scrolling ? 1 : 0);
}
...
Vt_selection_should_highlight_char(const Vt* const self, size_t x, size_t y)
{
    ...
    case SELECT_MODE_NORMAL:
        if (Vt_visual_top_line(self) +y > MIN(self->selection.begin_line,
                                              self->selection.end_line) &&
            Vt_visual_top_line(self) +y < MAX(self->selection.begin_line,
                                              self->selection.end_line))
        {
            return true;
        } else {
